...
impl ChipExt for ChipType {
    ...
    fn constraints(&self, coords: Coords, orient: Orientation)
                   -> Vec<PortConstraint> {
        let size = self.size();
        let ports = ports_internal(*self);
        constraints_internal(*self)
            .iter()
...
fn ports_internal(ctype: ChipType) -> &'static [AbstractPort] {
    match ctype {
        ChipType::Const(_) => {
...
fn constraints_internal(ctype: ChipType) -> &'static [AbstractConstraint] {
    match ctype {
        ...
        ChipType::Display => &[],
        ChipType::Button => &[AbstractConstraint::Exact(0, WireSize::Zero)],
    }
}

fn dependencies_internal(ctype: ChipType) -> &'static [(usize, usize)] {
    match ctype {
        ChipType::Const(_) |
        ChipType::Clock |
        ChipType::Delay |
        ...
        ChipType::Break => &[(0, 1)],
        ChipType::And | ChipType::Pack | ChipType::Join | ChipType::Sample => {
            &[(0, 2), (1, 2)]
        }
...
pub(super) fn new_chip_evals(ctype: ChipType, coords: Coords,
    ...
    debug_assert_eq!(slots.len(), ports_internal(ctype).len());
    match ctype {
        ChipType::Add => {
        ...
        ChipType::Join => {
            let chip_eval =
                eval::JoinChipEval::new(slots[0].0, slots[1].0, slots[2].0);
            vec![(2, chip_eval)]
        }
        ChipType::Latest => {
        ...
        ChipType::Sample => {
            let chip_eval =
                eval::SampleChipEval::new(slots[0].0, slots[1].0, slots[2].0);
            vec![(2, chip_eval)]
        }
        ChipType::Sub => {
