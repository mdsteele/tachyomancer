...
use crate::tachy::geom::{
    AsFloat, AsInt, Coords, CoordsRect, Direction, Orientation,
};
use crate::tachy::gui::{Sound, Ui};
use crate::tachy::save::{ChipType, WireShape};
use crate::tachy::state::{ChipExt, EditGrid, GridChange};
use cgmath::Point2;
...
impl ChipDrag {
    ...
    pub fn old_coords(&self) -> Option<Coords> {
        self.old_coords
    }

    pub fn new_orient(&self) -> Orientation {
    ...
    pub fn drop_onto_board(self, ui: &mut Ui, grid: &mut EditGrid) {
        ...
        changes.push(GridChange::AddChip(
            new_coords,
            self.chip_type,
            self.reorient * self.old_orient,
        ));
