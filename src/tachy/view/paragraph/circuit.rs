...
impl ParserPiece for ParserCircuitPiece {
    ...
    fn num_millis(&self) -> usize { self.num_millis }
    ...
    fn compile(&mut self, x_offset: f32, y_offset: f32) -> Box<CompiledPiece> {
        ...
        let chips: HashMap<Coords, (ChipType, Orientation)> = self.data
            .chips
            .iter()
            .map(|(coords, ctype, orient)| (coords + delta, (ctype, orient)))
            .collect();
        let mut fragments: HashMap<(Coords, Direction),
                                   (WireShape, usize)> = self.data
            .wires
            .iter()
            .map(|(coords, dir, shape)| ((coords, dir), (shape, usize::MAX)))
            .collect();
        ...
        let ports: HashMap<(Coords, Direction), (PortFlow, PortColor)> =
            chips
                .iter()
        ...
        let constraints: Vec<PortConstraint> = chips
            ...
            .collect();
        let _errors = state::determine_wire_sizes(&mut wires,
                                                  &wires_for_ports,
                                                  constraints);
...
impl CompiledPiece for CompiledCircuitPiece {
    ...
    fn add_x_offset(&mut self, x_offset: f32) { self.offset.x += x_offset; }

    fn add_y_offset(&mut self, y_offset: f32) { self.offset.y += y_offset; }

    fn draw(&self, resources: &Resources, paragraph_matrix: &Matrix4<f32>,
            _font_size: f32, millis_remaining: &mut usize)
            -> bool {
        if *millis_remaining < self.num_millis {
            return false;
        }
        *millis_remaining -= self.num_millis;
        let grid_matrix = paragraph_matrix * Matrix4::trans2v(self.offset) *
    ...
    fn debug_string(&self) -> String { "[Circuit]".to_string() }
}
